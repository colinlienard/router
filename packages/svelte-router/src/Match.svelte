<script lang="ts">
  ...
  const route: () => AnyRoute = () => router.routesById[routeId.current]
  ...
  if (matchState.current.match.status === 'pending') {
    pending = true
    const pendingMinMs =
    ...
    if (
      pendingMinMs &&
      !router.getMatch(matchState.current.match.id)?.minPendingPromise
    ) {
      ...
      if (!router.isServer) {
        ...
        Promise.resolve().then(() => {
          router.updateMatch(matchState.current.match.id, (prev) => ({
            ...prev,
            minPendingPromise,
          }))
        })
    ...
    router.getMatch(matchState.current.match.id)?.loadPromise?.resolve() // Resolve?
    pending = false
  }
</script>
...
{:else}
  ...
  {#if parentRouteId.current === rootRouteId}
    <OnRendered />
    <ScrollRestoration />
  {/if}
{/if}
